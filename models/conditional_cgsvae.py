...
class ConditionalEncoder(nn.Module):
    """The Encoder part of the Variational Autoencoder (VAE) that can optionally
    take conditional information.

    Encodes input data and optional conditional information into the parameters
    (mu, logvar) of a latent Gaussian distribution.

    Args:
        config (dict): A dictionary containing configuration parameters, including:
            - input_dim (int): Dimension of the input data.
            - condition_dim (int, optional): Dimension of the conditional input. Defaults to 0.
    ...
    def __init__(self, config):
        ...
        self.condition_dim = get_nested_config(config, ["model", "condition_dim"])
        check_type(self.condition_dim, int)
        ...
        if self.condition_dim > 0:
            prev_dim = self.input_dim + self.condition_dim
        else:
    ...
    def forward(self, x: torch.Tensor, c: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        Args:
            ...
            c (torch.Tensor, optional): Conditional input. Defaults to None.

        Returns:
            tuple: A tuple containing the mean (mu) and log variance (logvar) of the latent distribution.
        """
        if c is not None and self.condition_dim > 0:
            combined_input = torch.cat([x, c], dim=1)
        else:
            combined_input = x
        for layer in self.hidden_layers:
            combined_input = torch.relu(layer(combined_input))
        mu = self.mu_layer(combined_input)
        logvar = self.logvar_layer(combined_input)
        return mu, logvar


class ConditionalDecoder(nn.Module):
    """The Decoder part of the Variational Autoencoder (VAE) that can optionally
    take conditional information.

    Decodes a latent vector (z) and optional conditional information (c) into
    reconstructed data (x_recon) and an additional output (rn_percent) if configured.
    ...
    Attributes:
        ...
        rn_output_layer (nn.Linear, optional): Linear layer to output the additional output (rn_percent).
                                               Only present if additional_output_dim > 0.
    ...
    def __init__(self, config):
        ...
        self.logger = setup_logging(log_file_path=self.logdir, logger_object_name="Conditional_decoder")
        self.hidden_layers = nn.ModuleList()
        ...
        if self.condition_dim > 0:
            prev_dim = self.latent_dim + self.condition_dim
        else:
    ...
    def forward(self, z: torch.Tensor, c: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        ...
        Returns:
            tuple: A tuple containing the reconstructed data (x_recon) and the additional output (rn_percent).
                   rn_percent will be None if additional_output_dim is 0.
        """
        if c is not None and self.condition_dim > 0:
...
def _reparametrize(mu: torch.Tensor, logvar: torch.Tensor) -> torch.Tensor:
    ...
    Samples from the latent distribution q(z|x, c) using the provided mu and logvar.
...
class ConditionalCGSVAE(nn.Module):
    def __init__(self, config: dict, initialize_weights: bool = False):
        """Initializes the ConditionalCGSVAE model.
        ...
        """
        super(ConditionalCGSVAE, self).__init__()
        ...
        self.logdir = get_nested_config(config=config, keys=["run_info", "log_file_name"])
        self.logger = setup_logging(log_file_path=self.logdir, logger_object_name="ConditionalCGSVAE")
        self.config = config

        self.encoder = ConditionalEncoder(self.config).to(self.device)
        self.decoder = ConditionalDecoder(self.config).to(self.device)
    ...
    def forward(self, x: torch.Tensor, c: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, Optional[torch.Tensor], torch.Tensor, torch.Tensor]:
        ...
        x_recon, rn_percent = self.decoder(z, c)
