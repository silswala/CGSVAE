...
class Trainer:
    ...
    def __init__(self, model: torch.nn.Module, config):
        ...
        self.logger = setup_logging(log_file_path=self.logdir, logger_object_name="CGSVAE Trainer", level=logging.DEBUG)
    ...
    def train_model(self):
        ...
        for epoch in range(self.epoch):
            ...
            for batch_idx, (x_batch, rn_percentage_batch) in enumerate(self.train_dataloader):
                ...
                x_recon, rn_per, mu, logvar = self.model(x_batch)
                ...
                if "pf" in self.losses:
                    pf_loss = poisson_fidelity_loss(x_recon, x_batch).to(self.device)
                    if self.is_loss_clip:
                        pf_loss = torch.clamp(pf_loss, max=self.loss_max).to(self.device)
                    loss = loss + pf_loss
                    temp_losses_arr.append(pf_loss)
                    total_train_pf_loss += pf_loss
                    # loss_data["pf"] = pf_loss.item() / sample_size
                ...
                if torch.isnan(loss):
                    self.logger.warning(f"Skipping batch is NaN.")
                    continue
                if torch.isinf(loss):
                    self.logger.warning(f"Skipping batch is Inf.")
                    continue
            ...
            if "pf" in self.losses:
                loss_data["pf"] = total_train_pf_loss.item() / self.train_dataset_size
                train_msg += f"PF: {loss_data['pf']:.6f}, "
            if "rn" in self.losses:
            ...
            with torch.no_grad():
                ...
                if "pf" in self.losses:
                    loss_data["pf"] = total_test_pf_loss.item() / self.test_dataset_size
                    test_msg += f"PF: {loss_data['pf']:.6f}, "
                if "rn" in self.losses:
